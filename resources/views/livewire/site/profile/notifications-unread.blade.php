<div class="bg-white dark:bg-gray-700 p-8 rounded-md duration-500" id="comments-body">
    @if(count($notifications) > 0)
        @foreach($notifications as $item)
            {{ $item->markAsRead() }}
            <a href="{{ route('notification' , ['id' => $item->id]) }}" class="block bg-gray-100 dark:bg-gray-800 duration-500 rounded-md p-4 mb-2">
    ...
    @else
        <div class="flex flex-col justify-center items-center my-10">
            ...
            <p class="text-sm text-gray-400 text-center mt-5 dark:text-white">هیچ اعلان خوانده نشده ای یافت نشد!</p>
        </div>
    @endif
</div>
