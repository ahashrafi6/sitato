...
<div class="dir-rtl">
    ...
    <form method="POST" wire:submit.prevent="send">
               ...
               <div wire:ignore>
                   <select class="select2 form-control h-10" id="category">
                       ...
                       @foreach($categories as $item)
                           <option value="{{ $item->id }}" {{ $item->id == $category ? 'selected' : '' }}>{{ $item->fa_title }}</option>
                       @endforeach
                   </select>
               </div>
        ...
        <div class="grid grid-cols-12 gap-5 my-5">
            ...
            <div class=" {{ $access == 'cash' || $access == 'full' ? 'block' : 'hidden' }} col-span-12 lg:col-span-4">
                <div class="bg-white rounded-lg p-4 relative">
                    ...
                    <p class="mb-3">فایل بروزرسانی <span class="text-sm">(لایسنس نقدی)</span></p>
                    <ul class="text-xs text-gray-500">
                        <li class="mb-2">پسوند فایل: zip</li>
            ...
            <div
                class="{{ $access == 'subscribe' || $access == 'full' ? 'block' : 'hidden' }} col-span-12 lg:col-span-4">
                <div class="bg-white rounded-lg p-4 relative">
                    <span id="select-subscribe-file" class="cursor-pointer btn btn-gray absolute top-4 left-4 text-xs">
                        @if($subscribe_file)
                            تغییر فایل
                        @else
                            انتخاب فایل
                    ...
                    <p class="mb-3">فایل بروزرسانی <span class="text-sm">(لایسنس اشتراک)</span></p>
                    <ul class="text-xs text-gray-500">
                        <li class="mb-2">پسوند فایل: zip</li>
                        <li>حجم فایل: حداکثر 50 مگابایت</li>
                    </ul>
                    @if($subscribe_file)
                        ...
                        <span class="text-xs text-gray-500 mt-1">{{ $subscribe_file['name'] }}</span>
                    @endif
                    <x-auth.auth-validation-error name="subscribe_file"/>
                </div>
            </div>
        ...
        <div class="flex flex-col mt-8">
            ...
            <div>
                ...
                <button wire:loading.remove wire:target="send" type="submit" class="btn btn-primary">ذخیره محصول</button>
            </div>
        </div>
    </form>
